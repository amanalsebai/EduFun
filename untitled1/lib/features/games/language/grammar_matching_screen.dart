...
class _WordCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { Color cardColor = isMatched ? const Color(0xFF67E100) : Colors.white; Color borderAndNodeColor = isMatched ? const Color(0xFF3B8700) : (isSelected ? AppColors.primary : AppColors.primaryContainer); return GestureDetector(onTap: onTap, child: Column(mainAxisSize: MainAxisSize.min, children: [AnimatedContainer(duration: const Duration(milliseconds: 200), height: 66, width: double.infinity, alignment: Alignment.center, padding: const EdgeInsets.symmetric(horizontal: 10), decoration: BoxDecoration(color: cardColor, borderRadius: BorderRadius.circular(16), border: Border.all(color: borderAndNodeColor, width: isSelected || isMatched ? 3 : 1), boxShadow: isSelected ? [BoxShadow(color: AppColors.primaryContainer, blurRadius: 10)] : []), child: FittedBox(fit: BoxFit.scaleDown, child: Text(word, textAlign: TextAlign.center, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: isMatched ? Colors.white : AppColors.primary)))), const SizedBox(height: 6), Container(width: 22, height: 22, decoration: BoxDecoration(color: borderAndNodeColor, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 4)), child: isMatched ? const Icon(Icons.check, color: Colors.white, size: 10) : null)])); }
}
...
class _GrammarTermCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) { Color cardColor = isMatched ? const Color(0xFF67E100) : AppColors.secondaryContainer; Color borderAndNodeColor = isMatched ? const Color(0xFF3B8700) : (isSelected ? AppColors.secondary : AppColors.secondaryContainer); return GestureDetector(onTap: onTap, child: Column(mainAxisSize: MainAxisSize.min, children: [AnimatedContainer(duration: const Duration(milliseconds: 200), height: 66, width: double.infinity, alignment: Alignment.center, padding: const EdgeInsets.symmetric(horizontal: 8), decoration: BoxDecoration(color: cardColor, borderRadius: BorderRadius.circular(16), border: Border.all(color: borderAndNodeColor, width: isSelected || isMatched ? 3 : 1), boxShadow: isSelected ? [BoxShadow(color: AppColors.secondaryContainer, blurRadius: 10)] : []), child: Text(term, textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w900, color: isMatched ? Colors.white : AppColors.onSecondaryContainer))), const SizedBox(height: 6), Container(width: 22, height: 22, decoration: BoxDecoration(color: borderAndNodeColor, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 4)), child: isMatched ? const Icon(Icons.check, color: Colors.white, size: 10) : null)])); }
}