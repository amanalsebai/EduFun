...
class SpellingLevel {
  final String word; final String emoji; final List<String> scrambledLetters; final String hint;
  SpellingLevel({required this.word, required this.emoji, required this.scrambledWords, required this.hint}) : scrambledLetters = List.from(scrambledWords)..shuffle();
...
class _EnglishSpellingScreenState extends State<EnglishSpellingScreen> {
  ...
  Widget _buildMascotHint(SpellingLevel level) => Row(children: [const SizedBox(width: 80, height: 80, child: Center(child: Text("🤖", style: TextStyle(fontSize: 48)))), const SizedBox(width: 12), Expanded(child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white.withOpacity(0.85), borderRadius: BorderRadius.circular(16), border: Border.all(color: Colors.white.withOpacity(0.5), width: 2)), child: Text("تلميح: ${level.hint}", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.onSurface))))]);
